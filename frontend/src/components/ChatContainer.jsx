          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {messages.map(msg => (
              <div key={msg._id} className={`chat ${String(msg.senderId) === String(authUser._id) ? "chat-end" : "chat-start"}`}>
                <div className={`chat-bubble relative ${String(msg.senderId) === String(authUser._id) ? "bg-cyan-600 text-white" : "bg-slate-800 text-slate-200"}`}>
                  {msg.image && <img src={msg.image} className='rounded-lg h-48 object-cover' />}
                  {msg.text && <p className='mt-2'>{msg.text}</p>}
                  <p className='text-xs mt-1 opacity-75'>{new Date(msg.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                </div>
              </div>
            ))}
